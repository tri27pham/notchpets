...
**Stack:** SwiftUI + AppKit (NSPanel), SpriteKit for pet rendering, supabase-swift SDK (Postgres, Auth, Realtime), MediaRemote private framework for now-playing, Keychain for session storage, Sparkle for auto-updates.
...
Supabase schema is fully migrated. Magic link auth works. A user can generate an invite code and a second user can enter it to form a pair. Session is stored in the macOS Keychain and persists across restarts.
...
notchpets/
  Data/
    ...
    Models.swift              # Swift structs: Pet, Pair, Invite (Codable, matching schema)
  Storage/
...
Stage 1 is complete. Xcode project has supabase-swift package added.
Supabase project is initialised (supabase/ directory exists).
...
- Codable structs matching the schema: Pet, Pair, Invite
- Use snake_case CodingKeys where needed
- Pet.Species enum (cat, dog, frog, panda, penguin, rabbit) as String RawRepresentable
- Pet.Background enum (bedroom, rainy_window, forest, mount_fuji, cafe, beach, library, snowy_field)
...
- Otherwise: show PanelView (placeholder for now — Stage 1 panel)
...
*SpriteKit animation, backgrounds, and onboarding*

### Goal

Pets are rendered using SpriteKit with a full animation state machine. New users complete a setup wizard to choose species, name their pet, and pick a background. Pet data is written to Supabase and rendered live in the panel.
...
notchpets/
  Pet/
    ...
    AnimationState.swift     # AnimationState enum + auto-state logic
    ...
    PetSlotView.swift        # Updated: SpriteView wrapping PetScene (replaces placeholder)
  Data/
    PetStore.swift           # @MainActor ObservableObject: myPet, partnerPet, realtime sub
    PetRepository.swift      # Supabase queries: fetch pets, insert pet
  Setup/
    SetupWizard.swift        # 3-step wizard: species, name, background
    SpeciesPicker.swift      # Grid of pet previews (static first frame)
    BackgroundPicker.swift   # Grid of background scene thumbnails
  Assets.xcassets/           # Add spritesheet PNGs and background PNGs here
```
...
Spritesheets and backgrounds are not generated by Claude Code — source them from itch.io before starting this stage. Recommended search terms: `pixel art cat sprite sheet free`, `pixel art game backgrounds`. All assets must be CC0 or have a commercial-friendly licence. Add spritesheet PNGs to `Assets.xcassets` as Sprite Atlas assets and background PNGs as Image Sets before running the Stage 3 prompt.
...
- [ ] Both pets render via SpriteKit in the panel with correct backgrounds
- [ ] Idle animation plays continuously for both pets
- [ ] Animation state transitions are smooth with no frame flickering
- [ ] SpriteKit scene pauses when panel is collapsed
...
```
Build Stage 3 of notchpets: SpriteKit pet rendering, animation state machine, and setup wizard.

Stages 1 and 2 are complete. SupabaseManager, AuthManager, PetStore skeleton exist.
...
- init(species: String) — loads the species texture atlas from Assets.xcassets
- func setState(_ state: AnimationState, onComplete: (() -> Void)? = nil):
  - Builds SKAction.animate(with: frames, timePerFrame:) from the manifest
  - If loops: run repeatForever, else run once then call onComplete
  - Non-looping states (happy, eating, playing, dancing) call onComplete to return to idle

Create notchpets/Pet/PetScene.swift:
- Class PetScene: SKScene
- Properties: species, background, hunger, happiness, pendingTrigger: AnimationState?
- didMove(to view): sets background image node (fill frame), adds PetSpriteNode centred
- Sets initial state via autoState(hunger:happiness:)
- func trigger(_ state: AnimationState): calls petNode.setState — plays once then returns to autoState
- func updateStats(hunger: Int, happiness: Int): re-evaluates autoState if no triggered anim playing

Create notchpets/Pet/PetSlotView.swift (replace Stage 1 placeholder):
- SwiftUI View, props: species, background, hunger, happiness, triggerAnimation: AnimationState?
- SpriteView(scene: petScene) fixed at 180x150, .interpolation(.none)
- On triggerAnimation change: call petScene.trigger(_:)
- On hunger/happiness change: call petScene.updateStats(hunger:happiness:)
- Pauses scene when parent view is collapsed (via isPaused on scene or SKView)
...
- @Published var myPet: Pet? and @Published var partnerPet: Pet?
- @Published var myTrigger: AnimationState? and @Published var partnerTrigger: AnimationState?
...
- LazyVGrid of 6 species cards, each showing the idle first frame from the sprite atlas
- Highlight selected with a pixel art white border

Create notchpets/Setup/BackgroundPicker.swift:
- SwiftUI View, @Binding selection: String?
- LazyVGrid of 8 background thumbnails
- Highlight selected with a pixel art white border

Create notchpets/Setup/SetupWizard.swift:
- SwiftUI View, 3 steps tracked by @State step: Int
- Step 1: SpeciesPicker
- Step 2: Pet name TextField (12 char max)
...
- Observe PetStore via @EnvironmentObject
- If myPet == nil: show SetupWizard
- Otherwise: render two PetSlotViews with real pet data and trigger states

Swift 5.9, async/await. All SpriteKit operations on the main thread. No additional packages.
```
...
| **Animation trigger** | After writing to Supabase, the Realtime UPDATE event fires on both clients. PetStore infers `.eating` or `.playing` from the stat delta and sets the trigger. |
