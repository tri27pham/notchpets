...
## Stage 2 — Pet rendering & setup wizard
*SpriteKit animation, backgrounds, and onboarding*

### Goal

Pets are rendered using SpriteKit with a full animation state machine. New users complete a setup wizard to choose species, name their pet, and pick a background. Pet data is stored locally (UserDefaults) and rendered in the panel. No backend yet — Supabase wiring happens in Stage 3.
...
notchpets/
  ...
  Data/
    Models.swift             # Swift structs: Pet, Species, Background enums (Codable)
...
```
Build Stage 2 of notchpets: SpriteKit pet rendering, animation state machine, and setup wizard.

Stage 1 is complete. No backend yet — all pet data is stored locally via UserDefaults.
...
- Codable structs: Pet (id, name, species, background, hunger, happiness, etc.)
...
- func load() — reads pet from UserDefaults (JSON-encoded Pet)
- func save(_ pet: Pet) — writes pet to UserDefaults
- Stage 3 will replace local storage with Supabase and add partnerPet + Realtime
...
- On complete: create Pet with name, species, background, hunger 100, happiness 100.
  Save via PetStore.save(_:). Navigate to main panel.

Update notchpets/Views/PanelView.swift:
- Observe PetStore via @EnvironmentObject
- If myPet == nil: show SetupWizard
- Otherwise: render PetSlotView with real pet data
